#include <REGX52.H>

//引脚定义
sbit DS1302_SCLK =	P3^6;
sbit DS1302_IO 	 =	P3^4;
sbit DS1302_CE	 =	P3^5;


//寄存器写入地址/指令定义
#define DS1302_YEAR 	0x8C
#define DS1302_MONTH 	0x88
#define DS1302_DATE 	0x86

#define DS1302_HOUR 	0x84
#define DS1302_MINUTE 	0x82
#define DS1302_SECOND	0x80

#define DS1302_DAY		0x8A
#define DS1302_WP 		0x8E

//时间数组，索引0~6分别为年、月、日、时、分、秒、星期
extern  char DS1302_Time[]={23,11,1,0,0,0,1}; 


/**
  * @brief   将一个字节转换为BCD码形式并返回
  * @param   DATA 要转换的一个字节
  * @retval  BCD码形式的一个字节
  
*/
unsigned char BCD( char DATA)
{
	return (DATA/10*16 + DATA%10) ;
}


/**
  * @brief   将一个BCD码形式的字节转换为十进制并返回
  * @param   DATA 要转换的一个字节
  * @retval  十进制形式的一个字节
*/
unsigned char HEX(unsigned char DATA)
{
	return (DATA/16*10 + DATA%16) ;
}
 


/**
  * @brief   DS1302初始化
  * @param   无
  * @retval  无
  
*/
void DS1302_Init()
{
	DS1302_CE=0;
	DS1302_SCLK=0;
}




/**
  * @brief   DS1302写一个字节
  * @param   Command 命令字/地址
  * @param   Data 要写入的数据
  * @retval  无
  
*/
void DS1302_WriteByte(unsigned char Command,Data)
{
	unsigned char i;
	DS1302_CE=1;
	
	//模拟时序，将命令字送入DS1302
	for(i=0;i<8;i++)
	{
		DS1302_IO=Command & (0x01<<i);
		DS1302_SCLK=1;
		DS1302_SCLK=0;
	}
	
	//模拟时序，将要写入的数据送入DS1302
	for(i=0;i<8;i++)
	{
		DS1302_IO=Data & (0x01<<i);
		DS1302_SCLK=1;
		DS1302_SCLK=0;
	}
	
	DS1302_CE=0;

}




/**
  * @brief   DS1302读一个字节
  * @param   Command 命令字/地址
  * @retval  读出的数据
  
*/	
unsigned char DS1302_ReadByte(unsigned char Command)
{
	unsigned char i,Data=0x00;

	Command |= 0x01;  //将指令转换为读指令,写的地址加一就是读的地址
	DS1302_CE=1;
	
	//模拟时序，将命令字送入DS1302
	for(i=0;i<8;i++)
	{
		DS1302_IO=Command & (0x01<<i);
		DS1302_SCLK=0;
		DS1302_SCLK=1;
	}
	
	//模拟时序，读取DS1302送出的数据
	for(i=0;i<8;i++)
	{	
		DS1302_SCLK=1;
		DS1302_SCLK=0;
		
		if(DS1302_IO!=0)
		{
			Data |= (0x01<<i);
		}
	}
	
	DS1302_CE=0;
	DS1302_IO=0;  //读取后将IO设置为0，否则读出的数据会出错
	
	return Data;
}


/**
  * @brief   DS1302设置时间，调用之后，DS1302_Time数组的数字会被设置到DS1302中
  * @param   无
  * @retval  无
  
*/
void DS1302_SetTime()
{
	DS1302_WriteByte(DS1302_WP,0x00);  //WP置0：解除芯片写入保护
	
	DS1302_WriteByte (DS1302_YEAR,	BCD (DS1302_Time[0]));  //十进制转BCD码后写入
	DS1302_WriteByte (DS1302_MONTH,	BCD (DS1302_Time[1]));
	DS1302_WriteByte (DS1302_DATE,	BCD (DS1302_Time[2]));
	DS1302_WriteByte (DS1302_HOUR,	BCD (DS1302_Time[3]));
	DS1302_WriteByte (DS1302_MINUTE,BCD (DS1302_Time[4]));
	DS1302_WriteByte (DS1302_SECOND,BCD (DS1302_Time[5]));
	DS1302_WriteByte (DS1302_DAY,	BCD (DS1302_Time[6]));

	DS1302_WriteByte(DS1302_WP,0x80);  //打开写入保护
}



/**
  * @brief   DS1302读取时间，调用之后，DS1302中的数据会被读取到DS1302_Time数组中
  * @param   无
  * @retval  无
  
*/
void DS1302_ReadTime()
{
	DS1302_Time[0] = HEX (DS1302_ReadByte (DS1302_YEAR	 ));   //BCD码转十进制后读取
	DS1302_Time[1] = HEX (DS1302_ReadByte (DS1302_MONTH	 ));
	DS1302_Time[2] = HEX (DS1302_ReadByte (DS1302_DATE	 ));
	DS1302_Time[3] = HEX (DS1302_ReadByte (DS1302_HOUR	 ));
	DS1302_Time[4] = HEX (DS1302_ReadByte (DS1302_MINUTE ));
	DS1302_Time[5] = HEX (DS1302_ReadByte (DS1302_SECOND ));
	DS1302_Time[6] = HEX (DS1302_ReadByte (DS1302_DAY	 ));
	
}




